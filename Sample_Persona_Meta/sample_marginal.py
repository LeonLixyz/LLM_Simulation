...
Demographics = namedtuple('Demographics', ['Age', 'Sex', 'Race'])

def generate_personas_for_state(state):
    # Load data
    dp05 = f"./LLM_Simulation/Persona/US_Census_Data/DP05/structured_data/{state.abbr.lower()}_structured.json"
    dp02 = f"./LLM_Simulation/Persona/US_Census_Data/DP02/structured_data/{state.abbr.lower()}_dp02_structured.json"
    dp03 = f"./LLM_Simulation/Persona/US_Census_Data/DP03/structured_data/{state.abbr.lower()}_dp03_structured.json"
    s2301 = f"./LLM_Simulation/Persona/US_Census_Data/s2301/structured_data/{state.abbr.lower()}_s2301_structured.json"
    s2401 = f"./LLM_Simulation/Persona/US_Census_Data/s2401/structured_data/{state.abbr.lower()}_s2401_structured.json"
    ...
    # Sample demographics
    num_samples = 1000
    meta_persona_dir = f"./LLM_Simulation/Persona/Persona_State_Ultimate/{state.abbr}/meta_persona"
    personas = []
    for i in range(num_samples):
        with open(os.path.join(meta_persona_dir, f"persona_{i}.json"), 'r') as f:
            persona = json.load(f)['PERSONA']
            personas.append(Demographics(Age=persona['AGE'], Sex=persona['SEX'], Race=persona['RACE']))
    ...
    # Save personas
    base_dir = f"./LLM_Simulation/Persona/Persona_State_Ultimate/{state.abbr}/marginal_persona"
    ...
    for index, row in df.iterrows():
        ...
        with open(file_path, 'w') as f:
            json.dump({"PERSONA": row.to_dict()}, f, indent=2)
